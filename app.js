/* ============================================================
   ...
   flowDir — направление течения реки, градусы (0=С, 90=В, 180=Ю).
   Вписано вручную по географии. reservoirName — имя водохранилища.
   ============================================================ */
const HPPS = [
...
const S = {
  ...
  riverCourses:{}, reservoirsOSM:{}, loadingRiver:{}, loadingRes:{}
};
...
/* Ближайшая точка отрезка [a,b] к точке p, возвращает {t, point, dist} */
...
/* Найти ближайшую точку на полилинии к p */
...
/* ============================================================
   КЭШ
   ============================================================ */
const CACHE_KEY = 'ges_cache_v13';
const RIVER_KEY_PREFIX = 'ges_river_v4_';
const RES_KEY_PREFIX   = 'ges_res_v4_';
...
function writeCacheAll(o){ try{localStorage.setItem(CACHE_KEY,JSON.stringify(o));}catch(e){try{localStorage.removeItem(CACHE_KEY);}catch(_){}} }
function getCached(mode,id){ const a=readCacheAll(); const e=a[mode]?.[id]; if(!e)return null; if(Date.now()-e.ts>TTL[mode])return null; return e.data; }
function setCached(mode,id,data){ const a=readCacheAll(); if(!a[mode])a[mode]={}; a[mode][id]={ts:Date.now(),data}; writeCacheAll(a); }
...
/* ============================================================
   ЗАГРУЗКА РУСЛА
   Строим непрерывную линию от плотины ВНИЗ по течению (в сторону flowDir).
   ============================================================ */
...
out geom;`;
  ...
  try {
    ...
    // Все ways с их геометрией и расстоянием до плотины
    const ways = [];
    for (const el of (data.elements || [])){
      ...
      if (cl.dist < 5000){
        ways.push({ id: el.id, geom, closest: cl });
      }
    }
    if (!ways.length) throw new Error('нет рек рядом');

    // Сортируем по близости к плотине
    ways.sort((a,b) => a.closest.dist - b.closest.dist);

    // Разворачиваем каждый way так, чтобы он шёл ВНИЗ по течению:
    // «вниз» — это в сторону flowDir. Смотрим на направление от ближайшей
    // точки к концам way, сравниваем с flowDir.
    const flowRad = (h.flowDir || 180) * Math.PI / 180;
    const fLat = Math.cos(flowRad), fLon = Math.sin(flowRad);

    for (const w of ways){
      const g = w.geom;
      // Вектор от ближайшей точки к началу и к концу
      const p0 = w.closest.point;
      const dHead = [g[0][0]-p0[0], g[0][1]-p0[1]];
      const dTail = [g[g.length-1][0]-p0[0], g[g.length-1][1]-p0[1]];
      // Скалярное произведение с направлением течения
      const dotHead = dHead[0]*fLat + dHead[1]*fLon*Math.cos(h.lat*Math.PI/180);
      const dotTail = dTail[0]*fLat + dTail[1]*fLon*Math.cos(h.lat*Math.PI/180);
      // Разворачиваем так, чтобы «дальний от начала в сторону течения» был последним
      w.reversed = dotTail > dotHead;   // если хвост «более в сторону течения», way уже ориентирован
    }

    // Собираем цепочку: начинаем с way, ближайшего к плотине
    const used = new Set();
    let course = null;

    // Первый way
    const first = ways[0];
    let g0 = first.reversed ? [...first.geom].reverse() : [...first.geom];
    // Обрезаем от плотины: убираем часть до ближайшей точки
    const cl0 = closestOnLine([h.lat, h.lon], g0);
    let startIdx = cl0.idx;
    course = [ [h.lat, h.lon], ...g0.slice(startIdx + 1) ];
    used.add(first.id);
...
/* ============================================================
   ЗАГРУЗКА ВОДОХРАНИЛИЩА
   Фильтры:
   1) Имя совпадает с reservoirName (или содержит корень имени)
   2) Центроид НАД плотиной (в стороне -flowDir)
   3) Не дальше 25 км от плотины
   ============================================================ */
async function loadReservoirOSM(h){
  ...
  // Достаточно широкий радиус — некоторые водохранилища тянутся далеко
...
out geom;`;
  ...
  try {
    ...
    // Направление на «вверх по течению»
    ...
    // Корень имени водохранилища (без окончания «-ское», «-ое»)
    ...
    for (const el of (data.elements || [])){
      ...
      candidates.push({ parts, perim, dist, nameMatch, nm });
    }

    // Сортируем: сначала совпадение по имени, затем по периметру
    candidates.sort((a,b) => {
      if (a.nameMatch !== b.nameMatch) return b.nameMatch - a.nameMatch;
      return b.perim - a.perim;
    });

    // Оставляем до 3 объектов
...
/* ============================================================
   СИНТЕТИКА И OPEN-METEO (без изменений)
   ============================================================ */
function syntheticData(h, mode){
  let start, end;
  if (mode === 'history'){ start=new Date('2022-01-01'); end=new Date(Date.now()-8*864e5); }
...
async function fetchFromAPI(h, mode){
  if (mode === 'history'){
    const start='2022-01-01', end=new Date(Date.now()-8*864e5).toISOString().slice(0,10);
    ...
    while (queue.length){
      ...
      if (!S.markers[h.id]){
        const mk = L.circleMarker([h.lat, h.lon], {
          ...
          fillColor:'#3ddc84', fillOpacity: 1, opacity: 1
        }).addTo(layerMarkers);
...
/* ============================================================
   ПАРАМЕТРЫ ВОЛНЫ
   Уменьшены под реальный масштаб карты (30–50 км).
   ============================================================ */
...
/* ============================================================
   ВОДОХРАНИЛИЩЕ
   Рисуем ТОЛЬКО полигоны OSM (никаких самодельных эллипсов).
   ============================================================ */
function drawReservoir(h, i, lf, overtop){
  ...
  if (!osmPolys || !osmPolys.length) return;   // нет данных — ничего не рисуем
...
function buildUI(){
  ...
  $('slider').oninput = e => { S.idx = +e.target.value; if (S.scenario === 'flood') recompute(); render(); };
...
function updateChart(){
  ...
  S.chart.data.datasets = [
    { label:'Мощность, МВт', data:pw, borderColor:'#31c9ff', backgroundColor:'rgba(49,201,255,.12)', borderWidth:1.6, pointRadius:0, fill:true, yAxisID:'y', tension:.25 },
    { label:'Уровень ВБ, м', data:lv, borderColor:'#ffb02e', borderWidth:1.4, pointRadius:0, yAxisID:'y1', tension:.25 },
    { label:'Приток, м³/с', data:qq, borderColor:'#3ddc84', borderWidth:1, borderDash:[3,3], pointRadius:0, yAxisID:'y1', tension:.25, hidden:true }
  ];
  S.chart.update('none');
}
